...
//2
//package CH19_JDBC_MemberManageWithSwing_1027;
...
public class MemberDAO {
	...
	public MemberDTO getMemberDTO(int num) {
		...
		try {
			...
			con = getConn();
			String sql = "select * from member_info where num=?";
	...
	public Vector getMemberList() {
		...
		try {
			...
			con = getConn();
			String sql = "select * from member_info order by num asc";
			ps = con.prepareStatement(sql);
			rs = ps.executeQuery();
	...
	public boolean loginMemberCheck(String id, String pwd) {
	      ...
	      try {
	    	  con = getConn();
	          ps = con.prepareStatement("SELECT * FROM member_info WHERE id=? and pwd=?");
	...
	public boolean getIdByCheck(String id) {
	      ...
	      try {
	    	  con = getConn();
	          ps = con.prepareStatement("SELECT * FROM member_info WHERE id=?");
	          ps.setString(1, id.trim());
	          rs = ps.executeQuery(); //실행
	          if (rs.next())
	...
	public boolean insertMember(MemberDTO dto) {
		...
		try {
			...
			con = getConn();
		//String sql = "insert into member_info(" + "num,name,id,pwd,snid) " + "values(?,?,?,?,?)";
	String sql = "insert into member_info(" + "num,name,id,pwd,snid) " + "values(member_info_seq.NEXTVAL,?,?,?,?)";
//		String sql = "insert into member_info(" + "name,id,pwd,snid) " + "values(?,?,?,?)";
	...
	public boolean updateMember(MemberDTO vMem) {
		...
		try {
			...
			con = getConn();
			String sql = "update member_info set name=?, snid=? where id=? and pwd=?";
...
//			ps.setString(1, vMem.getNum());
	...
	public boolean deleteMember(String id, String pwd) {
		...
		try {
			con = getConn();
			String sql = "delete from member_info where id=? and pwd=?";
	...
	public void getUserSearch2(DefaultTableModel dt, String word) {
		...
		String sql = "SELECT * FROM member_info WHERE name" + " LIKE '%" + word.trim() + "%'" + "or id LIKE '%"+word.trim() + "%'";
...
//        PreparedStatement ps = null; //명령
	...
	public void getUserSearch(DefaultTableModel dt, String fieldName, String word) {
		...
		String sql = "SELECT * FROM member_info WHERE " + fieldName.trim() + " LIKE '%" + word.trim() + "%'";
